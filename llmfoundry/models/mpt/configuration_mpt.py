...
from llmfoundry.models.layers.attention import (check_alibi_support, get_flash_v2_version,
...
class MPTConfig(PretrainedConfig):
    ...
    def _validate_config(self) -> None:
        ...
        if self.attn_config['sliding_window_size'] != -1 and not (
                self.attn_config['attn_impl'] == 'flash' and
                is_flash_v2_installed(v2_version='2.3.0')):
            raise NotImplementedError(
                'sliding window only implemented with flash attention v2.3.0 or higher. '
                f"Found version: {get_flash_v2_version()}. \n"
                f"Flash Implementation: {self.attn_config['attn_impl']}. \n"
                f"- is_flash_v2_installed(v2_version='2.3.0'): {is_flash_v2_installed(v2_version='2.3.0')}\n"
                f"- self.attn_config['attn_impl'] == 'flash': {self.attn_config['attn_impl'] == 'flash'}"
            )
