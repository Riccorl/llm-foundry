...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name/path of the dataset (e.g., first argument to `datasets.load_dataset`)",
    )
    parser.add_argument(
        "--data_subset",
        type=str,
    ...
    parser.add_argument(
        "--skip-preprocessing",
        action="store_true",
        help="Whether to skip preprocesing (e.g., if the dataset is already formatted correctly)",
    )
    parser.add_argument(
        "--out_root",
        type=str,
...
def build_dataloader(
    dataset: SimpleDataset, batch_size: int, num_workers: Optional[int] = None
) -> DataLoader:
...
def main(args: Namespace) -> None:
    ...
    for split_name in args.splits:
        dataset = hf_datasets.load_dataset(
            path=args.dataset, name=args.data_subset, split=split_name, streaming=True
        )
        ...
        if args.local is not None:
            ...
            keep_local = False
        with MDSWriter(
            columns={key: "str" for key in columns},
            out=out,
            ...
            for sample in tqdm(samples, desc=split_name):
                ...
                if ("prompt" not in formatted_sample) or (
                    "response" not in formatted_sample
                ):
                    raise KeyError(
                ...
                encoded_sample = {
                    key: formatted_sample[key].encode("utf-8") for key in columns
                }
