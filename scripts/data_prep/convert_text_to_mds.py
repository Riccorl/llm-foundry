...
from llmfoundry.utils.data_prep_utils import DownloadingIterable, merge_shard_groups
...
def parse_args() -> Namespace:
    ...
    parser = ArgumentParser(
        description="Convert text files into MDS format, optionally concatenating and tokenizing",
    )
    parser.add_argument(
        "--output_folder",
        type=str,
...
def get_object_names(input_folder: str) -> List[str]:
    ...
    if object_store is not None:
        ...
        names = [
            name
            for name in object_store.list_objects(folder_prefix)
...
def download_and_convert(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        downloading_iter = DownloadingIterable(
            object_names=file_names, output_folder=tmp_dir, object_store=object_store
        )
        tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
        tokenizer.model_max_length = (
            5000000000  # Hack to prevent warnings from HuggingFace
        )
        ...
        with MDSWriter(
            out=output_folder, columns=columns, compression=compression
        ) as out:
...
def is_already_processed(
    output_root: str, args_str: str, object_names: List[str]
) -> bool:
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                output_object_store.download_object(
                    os.path.join(output_folder_prefix, DONE_FILENAME), done_file
                )
...
def convert_text_to_mds(
    ...
    if not reprocess and is_already_processed(output_folder, args_str, object_names):
        log.info(
    ...
    local_output_folder = (
        tempfile.TemporaryDirectory().name if is_remote_output else output_folder
    )
