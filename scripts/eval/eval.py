...
def load_peft_model(
    model_cfg: DictConfig, tokenizer: PreTrainedTokenizerBase, num_retries: int
) -> ComposerModel:
    try:
...
def evaluate_model(
    ...
    tokenizer_cfg: Dict[str, Any] = om.to_container(
        model_cfg.tokenizer, resolve=True
    )  # type: ignore
    ...
    loggers: List[LoggerDestination] = [
        build_logger(name, logger_cfg) for name, logger_cfg in loggers_cfg.items()
    ]
    ...
    if hasattr(model_cfg.model, "pretrained_lora_id_or_path"):
        ...
        composer_model = load_model(
            model_cfg.model, tokenizer, fsdp_config, num_retries
        )
...
def main(cfg: DictConfig) -> Tuple[List[Trainer], pd.DataFrame]:
    ...
    fsdp_config: Optional[Dict] = (
        om.to_container(fsdp_dict_cfg, resolve=True)
        if fsdp_dict_cfg is not None
        else None
    )  # type: ignore
    ...
    loggers_cfg: Dict[str, Any] = pop_config(
        cfg, "loggers", must_exist=False, default_value={}
    )
    icl_subset_num_batches: int = pop_config(
    ...
    for model_cfg in model_configs:
        ...
        model_results = calculate_markdown_results(
            logger_keys, trainer, benchmark_to_taxonomy, model_cfg.model_name
        )
        ...
        if eval_gauntlet_df is not None and eval_gauntlet_callback is not None:
            ...
            print(
                eval_gauntlet_df.sort_values(
                    list(eval_gauntlet_callback.averages.keys())[0], ascending=False
