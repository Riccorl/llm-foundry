...
def write_ft_checkpoint_from_composer_checkpoint(
    ...
    if (
        "integrations" not in composer_state_dict["state"]
        or "huggingface" not in composer_state_dict["state"]["integrations"]
    ):
        raise RuntimeError(
...
def parse_args() -> Namespace:
    ...
    parser = ArgumentParser(
        description="Convert an MPT Composer checkpoint into a standard FasterTransformer checkpoint folder."
    )
    parser.add_argument(
        "--composer_path",
        "-i",
