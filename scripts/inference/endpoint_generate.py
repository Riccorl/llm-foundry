...
import pandas as pd
import requests
from composer.utils import get_file, maybe_create_object_store_from_uri, parse_uri
...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        "--prompt-delimiter",
        default="\n",
        help="Prompt delimiter for txt files. By default, a file is a single prompt",
    )

    parser.add_argument(
        "-o", "--output-folder", required=True, help="Remote folder to save the output"
    )
    ...
    parser.add_argument(
        ...
        "--endpoint",
        type=str,
        help=f"OpenAI-compatible text completions endpoint to query on. If not set, will read from {ENDPOINT_URL_ENV}",
    )
...
async def main(args: Namespace) -> None:
    ...
    if not url:
        raise ValueError(f"URL must be provided via --endpoint or {ENDPOINT_URL_ENV}")
    ...
    log.info(f"Generating {len(prompt_strings)} prompts in {total_batches} batches")

    @sleep_and_retry
    @limits(calls=total_batches, period=1)  # type: ignore
    async def generate(session: aiohttp.ClientSession, batch: int, prompts: list):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i in range(total_batches):
            prompts = prompt_strings[
                i
                * args.batch_size : min((i + 1) * args.batch_size, len(prompt_strings))
            ]
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        output_object_store = maybe_create_object_store_from_uri(args.output_folder)
