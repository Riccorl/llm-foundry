...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        "--do_sample", type=str2bool, nargs="?", const=True, default=True
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_dtype", type=str, choices=["fp32", "fp16", "bf16"], default=None
    )
    parser.add_argument(
...
def main(args: Namespace) -> None:
    ...
    try:
        config = AutoConfig.from_pretrained(args.name_or_path, **from_pretrained_kwargs)
    ...
    for temp, topp, topk, repp, nrnz, seed in itertools.product(
        ...
        if args.max_batch_size:
            ...
            batches = [
                prompt_strings[i : i + bs] for i in range(0, len(prompt_strings), bs)
            ]
        ...
        for batch in batches:
            ...
            gen_tokens = torch.sum(encoded_gen != tokenizer.pad_token_id, axis=1).numpy(
                force=True
            )  # type: ignore
            ...
            effective_prompts = tokenizer.batch_decode(
                encoded_inp["input_ids"], skip_special_tokens=True
            )
