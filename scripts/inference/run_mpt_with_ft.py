...
def main():
    ...
    parser.add_argument("--layer_num", type=int, default=32, help="number of layers")
    parser.add_argument(
        "--input_len", type=int, default=128, help="input sequence length to generate."
    )
    parser.add_argument(
        "--output_len", type=int, default=64, help="output sequence length to generate."
    )
    parser.add_argument("--head_num", type=int, default=32, help="head number")
    parser.add_argument("--size_per_head", type=int, default=128, help="size per head")
    parser.add_argument("--vocab_size", type=int, default=50432, help="vocab size")
    parser.add_argument(
        "--beam_width",
        type=int,
        default=1,
    ...
    parser.add_argument(
        "--top_p", type=float, default=0.95, help="top p probability threshold"
    )
    ...
    parser.add_argument(
        ...
        help="Name of the tokenizer or the directory where the tokenizer file is located.",
    )
    parser.add_argument(
        "--lib_path",
        type=str,
    ...
    parser.add_argument("--start_id", type=int, default=0, help="start token id.")
    parser.add_argument("--end_id", type=int, default=0, help="end token id.")
    parser.add_argument(
        "--max_batch_size",
        type=int,
        default=8,
        help="Max batch size. If sample_input_file is given, it is truncated to this max_batch_size, otherwise, this value is used as batch size.",
    )
    parser.add_argument(
        "--repetition_penalty", type=float, default=5.0, help="repetition penalty"
    )
    parser.add_argument(
        "--presence_penalty",
        type=float,
        default=0.0,
        help="presence penalty. Similar to repetition, but additive rather than multiplicative.",
    )
    parser.add_argument(
        "--min_length",
        type=int,
    ...
    if rank == 0:
        if not args.use_gpt_decoder_ops:
            ...
            cum_log_probs = (
                gen_outputs["cum_log_probs"] if return_cum_log_probs > 0 else None
            )
        ...
        for i, (context, tokens) in enumerate(zip(contexts, tokens_batch)):
            for beam_id in range(beam_width):
                ...
                output = (
                    ...
                    if args.detokenize
                    else " ".join(str(t) for t in token.tolist())
                )
