...
def upload(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if revision is not None:
            logger.info(f"Creating branch {revision}")
            api.create_branch(repo_id, repo_type=repo_type, branch=revision)
        ...
        if archive:
            ...
            if filenames is not None:
                ...
                if model_dir.is_dir():
                    ...
                    os.system(f"cp {model_dir} {tmpdir}")
        ...
        api.upload_folder(
            ...
            repo_type=repo_type,
            revision=revision,
            commit_message=commit or "Automatic push from sapienzanlp",
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--revision",
        help="The revision of the model you want to upload. If not provided, the latest revision will be used.",
    )
    parser.add_argument(
        "--repo_type",
        default="model",
        help="The type of the repository you want to create. Default is 'model'.",
    )
