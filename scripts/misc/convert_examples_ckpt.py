...
def convert_examples_ckpt(
    ...
    if "huggingface" in composer_state_dict["state"]["integrations"]:
        ...
        composer_state_dict["state"]["integrations"]["huggingface"]["model"]["config"][
            "content"
        ] = hf_config
    ...
    if "optimizers" in composer_state_dict["state"].keys():
        ...
        for opt in composer_state_dict["state"]["optimizers"].keys():
            ...
            for pg_idx in range(
                len(composer_state_dict["state"]["optimizers"][opt]["param_groups"])
            ):
                for param_idx in range(
                    ...
                    param_name = composer_state_dict["state"]["optimizers"][opt][
                        "param_groups"
                    ][pg_idx]["params"][param_idx]
                    for old, new in conversion_dict.items():
                        param_name = param_name.replace(old, new)
                    composer_state_dict["state"]["optimizers"][opt]["param_groups"][
                        pg_idx
                    ]["params"][param_idx] = param_name

    # Save weights
    file_path = str(Path(local_output_path) / str(checkpoint_path).split("/")[-1])
    print(f"Writing converted output to {file_path}")
    torch.save(composer_state_dict, file_path)

    if object_store is not None:
        remote_file_path = os.path.join(
            local_folder_path, str(checkpoint_path).split("/")[-1]
        )
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Convert ckpt created with the examples repo into one usable by llmfoundry."
    )
