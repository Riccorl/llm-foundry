...
def parse_args():
    ...
    parser.add_argument("-s", "--save-path", type=str, default="benchmark_results")
    parser.add_argument(
        "-p", "--print-results", type=str_to_bool, nargs="?", const=True, default=False
    )
...
def get_runs(args: argparse.Namespace):
    runs = [
        r
        for r in msdk.get_runs(include_details=True)
        if args.project in r.name.split("-")[0]
        and r.status == msdk.RunStatus("COMPLETED")
    ]
...
def parse_run(run: msdk.Run) -> Dict[str, Any]:
    ...
    mfu_w_attn = (
        (3 * flops_per_seq + 3 * attn_flops_per_seq)
        * throughput
    ...
    if activation_checkpointing:
        hfu_w_attn = (
            (4 * flops_per_seq + 4 * attn_flops_per_seq)
            * throughput
            / (gpus * GPU_AVAILABLE_FLOPS)
        )
    else:
    ...
    return {
        ...
        "Model TFLOP": model_tflop,
        "MicroBatchSize": micro_batchsize,
        "GradAccum": math.ceil(global_train_batch_size / gpus / micro_batchsize),
        "GlobalBatchSize": global_train_batch_size,
        "Throughput (S/s)": int(throughput),
        "Throughput (T/s)": int(throughput * seq_len),
        "Throughput (T/s/GPU)": int(throughput * seq_len / gpus),
        "GlobalBatchSize (T)": global_train_batch_size * seq_len,
        "Precision": run.submitted_config.parameters["precision"],
