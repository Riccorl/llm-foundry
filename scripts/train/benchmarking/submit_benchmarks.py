...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate and run configurations to test MPT training throughput on the MosaicML platform."
    )
    ...
    parser.add_argument(
        "--git_branch", type=str, default=None, help="what git branch to use."
    )
    parser.add_argument(
        "--git_commit", type=str, default=None, help="what git commit to use."
    )
    parser.add_argument(
        "-t",
        "--precisions",
    ...
    parser.add_argument(
        "--fsdp_config_shard_strategy", type=str, nargs="?", const=True, default=None
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--fsdp_config_backward_prefetch", type=str, nargs="?", const=True, default=None
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--gpu_types", type=str, default=["a100_40gb"], nargs="+", choices=_gpu_types
    )
    known_args = parser.parse_known_args()[0]
    _gpu_nums = get_gpu_nums(known_args.clusters, known_args.gpu_types)
    parser.add_argument(
        "-g", "--gpu_nums", type=int, default=[8], nargs="+", choices=_gpu_nums
    )
    ...
    parser.add_argument(
        "--wandb", type=str_to_bool, nargs="?", const=True, default=True
    )

    parser.add_argument("--priority", type=str, default="lowest")

    parser.add_argument("--RUN", type=str_to_bool, nargs="?", const=True, default=False)
...
def get_global_train_batch_sizes(
    max_seq_len: int, pows: List[int], batch_sizes: Optional[List[int]] = None
):
...
def mod_parameters(
    ...
    if data_remote is not None:
        ...
        parameters["train_loader"]["dataset"][
            "split"
        ] = "train_small"  # for throughput testing purposes
        parameters["eval_loader"]["dataset"][
            "split"
        ] = "val_small"  # for throughput testing purposes
    # set max_seq_len
    ...
    if pad_vocab_multiple:
        ...
        parameters["model"]["vocab_size"] = (
            math.ceil(vocab_size / pad_vocab_multiple) * pad_vocab_multiple
        )
    ...
    parameters["device_eval_batch_size"] = max(
        1, int(parameters["device_eval_batch_size"] / ((max_seq_len / 2048) ** 2))
    )
...
def get_integrations(
    ...
    if wandb:
        integrations += [
            {"integration_type": "wandb", "entity": "mosaic-ml", "project": project}
        ]
...
def run_config(
    config: Tuple[str, int, int, str, str, int, str], args: argparse.Namespace
):
    (
        model_yaml,
        max_seq_len,
    ...
    integrations = [
        ...
        {"integration_type": "wandb", "entity": "mosaic-ml", "project": args.project},
    ]

    command = ""
    if (
        gpu_type == "h100_80gb" and "fp8" in precision
    ):  # Required for flash-attn and FP8 training
        command += f"""
    ...
    config = RunConfig(
        name=name,
        compute={"cluster": cluster, "gpu_type": gpu_type, "gpus": gpu_num},
        image=args.image,
...
def run_check_capacity(
    model_yaml: str, gpu_num: int, gpu_type: str, p_multiplier: int = 16
):
        ...
        for cluster in args.clusters:
            for gpu_type in get_cluster_gpu_types(cluster):
                ...
                for gpu_num in _gpu_nums:
                    ...
                    if (
                        not global_train_batch_sizes
                        and args.microbatch_size is not None
                    ):
                        ...
                        for precision in args.precisions:
                            for model_yaml in args.model_yamls:
                                run = run_check_capacity(
                                    model_yaml, gpu_num, gpu_type, p_multiplier=4
                                )
